...
const ArtikelDetail = () => {
  ...
  const renderContent = (content) => {
    ...
    return content.split('\n').map((paragraph, index) =>
      paragraph ? (
        <p key={index} className="mb-4 break-words break-all">{paragraph}</p>
      ) : (
        <br key={index} />
      )
    );
  ...
  return (
    <div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h1 className="text-4xl sm:text-5xl font-bold mb-8 break-words">{artikel.judul || "Artikel"}</h1>
          ...
          {artikel.gambar ? (
            <img
              ...
              onError={(e) => {
                ...
                e.target.parentElement.style.background = "repeating-conic-gradient(#f3f4f6 0% 25%, #ffffff 0% 50%) 50% / 40px 40px";
            ...
            <div
              className="w-full"
              style={{
                background: "repeating-conic-gradient(#f3f4f6 0% 25%, #ffffff 0% 50%) 50% / 40px 40px",
                minHeight: "300px"
              }}
            ></div>
          )}
        ...
        {/* Content */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold mb-4 break-words">{artikel.judul}</h2>

          <div className="prose prose-lg max-w-none break-words break-all">
            {artikel.bio &&
              artikel.bio.split(/\r?\n/).map((line, index) =>
                line ? (
                  <p key={index} className="mb-2 break-words break-all">{line}</p>
                ) : (
                  <br key={index} />
                )
              )}
